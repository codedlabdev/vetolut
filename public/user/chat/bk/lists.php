        ...
        <div class="chat-scroll px-3 pb-3 overflow-auto">
            <div class="d-flex align-items-center justify-content-between">
                ...
                <?php foreach ($formattedPinnedPartners as $partner): ?>
                    <a href="chat.php?partner=<?= $partner['id'] ?>" class="link-dark text-center">
                ...
                <?php if (empty($formattedPinnedPartners)): ?>
                    <p class="text-center text-muted d-flex justify-content-center align-items-center w-100">No pinned chats</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
    ...
    ?>
            <div class="bg-white shadow-sm d-flex align-items-center gap-3 p-3 border-bottom position-relative">
                <div class="position-absolute end-0 top-0 p-2">
                    <button class="btn btn-link p-0" data-bs-toggle="modal" data-bs-target="#chatOptionsModal<?= $partner['id']; ?>">
                        <i class="fas fa-ellipsis-h" style="font-weight: bold; font-size: 1.5em;"></i>
                    </button>
                </div>

                <a href="<?php echo BASE_URL . 'user/chat/inbox.php?id=' . $partner['id']; ?>" class="d-flex align-items-center flex-grow-1 text-decoration-none">
                    <!-- Display partner's profile picture -->
                    <img src="<?= $partner['profileImage']; ?>" alt="" class="img-fluid rounded-pill message-profile me-3">
            ...
            <!-- Chat Options Modal -->
            <div class="modal fade" id="chatOptionsModal<?= $partner['id']; ?>" tabindex="-1" aria-labelledby="chatOptionsLabel<?= $partner['id']; ?>" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="chatOptionsLabel<?= $partner['id']; ?>">Chat Options (<?= htmlspecialchars($partner['fullName']); ?>)</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="list-group">
                                <?php if (isset($partner['pinnedAt'])): ?>
                                    <a href="#" class="list-group-item list-group-item-action" onclick="unpinChat(<?= $partner['id']; ?>)">
                                        <i class="fas fa-thumbtack pin-icon" style="margin-right: 13px;"></i> Unpin Chat
                                    </a>
                                <?php else: ?>
                                    <a href="#" class="list-group-item list-group-item-action" onclick="pinChat(<?= $partner['id']; ?>)">
                                        <i class="fas fa-thumbtack pin-icon" style="margin-right: 13px;"></i> Pin Chat
                                    </a>
                                <?php endif; ?>
                                
                                <a href="#" class="list-group-item list-group-item-action text-danger" onclick="deleteChat(<?= $partner['id']; ?>)">
